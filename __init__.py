...
def create_app():
    ...
    def admin_stats():
        ...
        db.close()
        return jsonify({'users': user_list, 'sys_cpu': f"{total_cpu}%"})
    ...
    def server_action(folder, act):
        ...
        if act in ['start', 'restart']:
            ...
            proc = subprocess.Popen(['python3', startup_file], cwd=path, stdout=f_log, stderr=f_log, preexec_fn=preexec_fn)
    ...
    def delete_server(folder):
        ...
        if srv_data['server_status'] == 'suspended':
            db.close()
