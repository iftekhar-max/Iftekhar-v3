import os, sqlite3, zipfile, subprocess, signal, shutil, psutil, time, datetime, sys
    ...
    def server_action(folder, act):
        ...
        if act == 'install':
            ...
            if os.path.exists(req_path):
                ...
                f_log.flush()
                subprocess.Popen([sys.executable, '-m', 'pip', 'install', '-r', 'requirements.txt'], cwd=path, stdout=f_log, stderr=f_log)
                db.close()
        ...
        if act in ['start', 'restart']:
            ...
            proc = subprocess.Popen([sys.executable, startup_file], cwd=path, stdout=f_log, stderr=f_log, preexec_fn=os.setsid)
